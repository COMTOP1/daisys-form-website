...
export default function Sidebar() {
  ...
  //     await signOut({ callbackUrl: process.env.NODE_ENV === "development" && process.env.IS_OFFLINE === "true" ? "/auth/login" : "/auth/signin" });
  //   } catch (err) {
  ...
  const separatorStyle = "truncate text-gray-400 p-4";
  ...
  return (
    <>
      ...
      {!isOpen && (
        <div className={"lg:hidden h-16 min-w-screen bg-gray-50"}>
          <button
            ...
            className="lg:hidden fixed top-3 left-3 z-50 baseColour text-white p-2 rounded-lg shadow-md hover:bg-gray-800 transition"
            aria-label="Open menu"
          >
            <Menu size={22} />
          </button>
          <h1 className={"text-2xl p-4 text-center"}>
            <span className={"font-extrabold"}>Daisy&#39;s</span> Admin
          </h1>
        </div>
      )}
      ...
      {/* --- Sidebar --- */}
      <aside
        className={`fixed lg:static top-0 left-0 min-h-screen w-64 baseColour text-gray-200 flex flex-col justify-between shadow-xl transform transition-transform duration-300 ease-in-out z-40
      ...
      >
        ...
        <div className="flex items-center justify-between h-14 px-4">
          <div className="flex items-center gap-3">
            <Image
              src={"/daisys-logo-with-background.jpg"}
              alt={"Daisy's logo"}
              width={32}
              height={32}
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-1">
          ...
          <Link href="/admin/availableDates" className={linkStyle}>
            <Calendar className={iconStyle} />
            <span className="truncate">All Available Dates</span>
          </Link>
